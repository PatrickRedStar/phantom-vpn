...
final class AdminManager: ObservableObject {
    ...
    func createClient(name: String, expiresDays: Int?) {
        Task { @MainActor in
            do {
                ...
                if let days = expiresDays { body["expires_days"] = days }
    ...
    private func fetchClients() async throws -> [AdminClientInfo] {
        ...
        return (arr as? [[String: Any]] ?? []).map { o in
            AdminClientInfo(
                ...
                expiresAt: (o["expires_at"] as? NSNumber).flatMap { n in
                    let v = n.int64Value
                    return v > 0 ? v : nil
                }
            )
    ...
    private func request(_ method: String, path: String, body: Data? = nil) async throws -> Data {
        ...
        if code < 200 || code >= 300 {
            throw NSError(domain: "AdminAPI", code: code, userInfo: [NSLocalizedDescriptionKey: "HTTP \(code)"])
        }
        return data
    }
}
