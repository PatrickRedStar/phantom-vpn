...
## Lessons learned (первая итерация CI)

Первый push в CI стоил 4 неудачных билда подряд — каждый чинил очередной симптом, не понимая корень:

1. **Cargo workspace resolve читает Cargo.toml ВСЕХ members** до любой команды, даже `cargo build -p phantom-server`. Если member отсутствует — fail. Изначальный `.dockerignore` исключал `crates/client-android`, `crates/client-apple`, `crates/client-core-runtime`, `crates/gui-ipc`, весь `apps/` — все они в workspace.members корневого `Cargo.toml`. Cargo не находит их manifests → `No such file or directory`.

2. **Dockerfile должен COPY все workspace member пути**, не только нужные для конкретного `-p`. `COPY crates/ server/` мало; нужно ещё `COPY apps/linux/` и `COPY apps/openwrt/` (это workspace members). Mobile/desktop клиенты (`apps/android`, `apps/macos`, `apps/ios`, `apps/windows`) НЕ в workspace — их можно исключить через .dockerignore.

3. **Rust 1.83 устарел** для transitive deps в Cargo.lock. `time-macros 0.2.27` использует Cargo feature `edition2024`, стабилизированную в Rust 1.85. Бамп до `rust:1-bookworm` (latest stable) решает раз и навсегда.

4. **Local build обязателен перед CI**. Docker Desktop на macOS делает `--platform linux/amd64` через Linux VM нативно (НЕ QEMU). Build server +relay ≈ 3-4 мин. CI feedback loop = 1+ мин до error → loop'ы фиксов экономически невыгодны. Скрипт `docker/build-local.sh` собирает оба + smoke-test bootstrap (CA + server.toml + clients.json + проверка `[h2]` секции). **Запускать перед каждым push'ем, который трогает docker/ или workspace.**

5. **Plan-level oversight**: план Task 2 / Task 6 предполагал что `cargo build -p X` нужны только `crates/` + `server/`. Это было неверным предположением, оставленным в плане без локальной валидации. Final reviewer тоже отложил "first real test will be in CI" вместо запуска `docker buildx build`. Урок: для Rust workspace в Docker — local build обязателен до review.

## Связанные документы

- План — `docs/superpowers/plans/2026-06-04-docker-init-server-relay.md`.
- Build процедура — [../build.md](../build.md) (раздел "Docker deploy").
- Local build validator — `docker/build-local.sh` (запускать перед push'ем).
