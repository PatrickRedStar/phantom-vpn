...
| `v0.24.0` | **Android: honest state + TSPU-resilient reconnect + structured logs.** Полный ADR — [`0009`](../decisions/0009-android-honest-state-and-resilience.md). Шорт: (1) StatusFrame расширен `last_rx_ms/last_tx_ms/idle_rx_secs/health/bandwidth_class`, Telemetry стэмпит timestamps + derive Stale/Throttled hysteresis-based. (2) `tls_rx_loop` получает `idle_timeout=45s` — ломает half-open zombie. (3) `applyUnderlyingNetworks()` на establish + NetworkCallback fires — Android видит реальный физ-network тоннеля. (4) Per-category error backoff (HardReset=1s, IdleTimeout=2s, …); `BACKOFF_SECS` → `[1,2,5,10,20,30,30,30]`. (5) Kotlin `derivedVpnState` = `combine(lifecycle, StatusFrame)` → UI states `Stale/Throttled/Reconnecting` с правильным цветом/тикером, мигалка multiplex только при `rateRxBps > 0`. (6) Logs UI: persistent rotating file ring buffer 5×2MB, search, category chips, expandable fields, share = весь on-disk log. `versionCode 65 → 66`. Hot failover отложен в Phase 2. |
